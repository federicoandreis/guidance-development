...
After reading this chapter, you will know when to apply a t-test versus z-scoring or SPC, how to carry out one-sample, two-sample, and paired t-tests step by step, interpret results in terms of both statistical and practical significance, and understand when assumption violations require an alternative approach.
...
- **z-tests**: Assume you know the population standard deviation (σ)
...
**QA Framework alignment**: t-tests most directly support QA question 3 (Analysis — is the method appropriate and correctly applied?). When used for pilot evaluations, they also support question 5 (Fitness for Purpose — does the comparison answer the right question?). Document assumption checks, effect size calculations, and the choice between one-sample, two-sample, or paired test as part of the question 3 record.
...
- **Comparing >2 groups**: Use ANOVA (beyond this guide — raise with the Guild)
...
[View Example 4: Medication Safety Pilot →](../examples/example-04-complete.qmd)
...
[View Example 5: Regional A&E Performance →](../examples/example-05-complete.qmd)
:::
...
**Null hypothesis (H₀)**: The group mean equals the target value (μ = μ₀)

**Alternative hypothesis (H₁)**: The group mean differs from the target value (μ ≠ μ₀)

**Example**: Testing if average waiting time equals 4-hour target
- H₀: Mean waiting time = 4.0 hours
- H₁: Mean waiting time ≠ 4.0 hours
...
Subtract the target from the sample mean, then divide by the standard error (standard deviation divided by √n).
...
- p ≥ 0.05: Fail to reject null hypothesis (insufficient evidence of difference)

**Confidence interval**: 95% CI for the mean
- If CI doesn't include target value → significant difference
- If CI includes target value → no significant difference
...
**Null hypothesis (H₀)**: Both groups have the same mean (μ₁ = μ₂)

**Alternative hypothesis (H₁)**: Groups have different means (μ₁ ≠ μ₂)

**Example**: Comparing waiting times between two regions
- H₀: Mean waiting time in Region A = Mean waiting time in Region B
- H₁: Mean waiting time in Region A ≠ Mean waiting time in Region B
...
Divide the difference between group means by the pooled standard error.
...
Weight each group's variance by its degrees of freedom, combine them, then take the square root.
...
Express the difference between means in units of pooled standard deviation.
...
**Null hypothesis (H₀)**: Mean difference between paired measurements is zero (μ_d = 0)

**Alternative hypothesis (H₁)**: Mean difference is not zero (μ_d ≠ 0)

**Example**: Testing if satisfaction scores changed after intervention
- H₀: Mean change in satisfaction = 0
- H₁: Mean change in satisfaction ≠ 0
...
Divide the mean difference by the standard error of the differences.
...
- p ≥ 0.05: Fail to reject null hypothesis (insufficient evidence of change)

**Confidence interval**: 95% CI for the mean difference
- If CI doesn't include 0 → significant change
- If CI includes 0 → no significant change

**Effect size**: Cohen's d for paired data
$$d = \frac{\bar{d}}{s_d}$$

Express the mean difference in units of its own standard deviation.
...
**Observation**: Pilot practices have higher mean scores (difference ≈ 7.5 points) with slightly more variability.
...
- Independent t-test for paired data → wrong
- Paired t-test for independent data → wrong
...
## ⚙️ Overthinking: Assumption Violations and Alternatives
...
- Raise with the Guild if unsure which alternative applies
...
**If violated**: Don't use standard t-test. Raise with the Guild for guidance on multilevel models or repeated-measures methods.
...
**When to escalate**: If you're unsure whether assumptions are met, or how to handle violations, raise this at a Guild Quant Clinic session before proceeding. Getting this wrong invalidates your results.
:::

## Related Approaches

### For different analytical questions
- **Many providers to compare** → Z-scoring (@sec-z-scoring) is more appropriate
- **Monitoring over time** → SPC (@sec-spc-basics) for time series
- **Before/after with time series** → SPC with intervention point (Example 2)
- **Non-normal data** → See Overthinking box above for alternatives

### For advanced comparisons
- **More than two groups** → ANOVA (beyond this guide — raise with the Guild)
- **Confounding variables** → ANCOVA or regression (raise with the Guild)
- **Repeated measures** → Mixed models (raise with the Guild)

### For foundational concepts
- **Understanding variation** → @sec-variation-uncertainty
- **Effect sizes and power** → @sec-variation-uncertainty (Overthinking boxes)
- **Data quality checks** → @sec-qa-principles

### For complete workflows
- **Medication Safety Pilot** → Example 4 (paired t-test)
- **Regional A&E Performance** → Example 5 (two-sample t-test)
...
- **Complex study designs**: Crossover designs, cluster randomisation, or other specialised designs
- **Non-parametric alternatives**: Data doesn't meet assumptions and you need Mann-Whitney or Wilcoxon tests
- **Power calculations**: Need to determine required sample size before conducting a study
...
6. **Statistical significance ≠ practical significance**: Always consider context
7. **When in doubt, raise with the Guild**: Especially for assumption violations or complex designs
