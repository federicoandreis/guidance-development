...
After reading this chapter, you will understand how statistical inference moves from observed data to defensible conclusions, know how to interpret p-values and confidence intervals correctly, distinguish statistical significance from practical significance using effect sizes, and apply appropriate thresholds given CQC's regulatory priorities around false negatives versus false positives.
...
Most CQC analysis is **exploratory**: data is used to understand patterns and guide decisions, not to provide definitive proof.
...
## ⚙️ Overthinking: Statistical Power
...
**When to escalate**: If designing a new indicator and unsure about power calculations, seek specialist support.
:::
...
- To apply inference to provider comparisons using z-scores → @sec-z-scoring
- To apply inference to time series (control limits as inference) → @sec-spc-basics
- To test differences between two groups using t-statistics → @sec-t-tests
- To test relationships between variables → @sec-correlation
- To understand how missing data undermines valid inference → @sec-missing-data-foundations
- For documenting analytical decisions and uncertainty → @sec-qa-principles
:::
