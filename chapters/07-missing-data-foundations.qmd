...
After reading this chapter, you will understand the three missing data mechanisms (MCAR, MAR, MNAR), know how to investigate which applies to your dataset, and be able to decide when simple complete case analysis is appropriate and when the pattern of missingness is itself the most important finding.
...
- **Easy to handle**: Analysing complete cases only is valid
...
**Example**: You plan to compare 100 providers, but 20 have missing data. Analysing 80 providers instead of 100 means less precise estimates—wider confidence intervals, lower statistical power.
...
- For high-stakes analysis, consult the Quantitative Analytics and Statistics Guild (the Guild) on imputation methods
...
#### Action
1. **Seek specialist support**: This requires specialist methods or may indicate indicator isn't fit for purpose
...
3. **Specialist support**: Specialist methods exist (selection models, pattern-mixture models) but require strong assumptions and expertise
...
## ⚙️ Overthinking: The MAR Assumption Is Often Unverifiable
...
- For computational implementation (multiple imputation, sensitivity analysis, R and Python code) → @sec-missing-data-handling
- For understanding how missingness affects statistical inference and uncertainty → @sec-statistical-inference
- For data quality checks before analysis → @sec-qa-principles
- For handling unusual values that may indicate data errors → @sec-unusual-observations
- For small denominator problems that can look like missingness → @sec-z-scoring (Overthinking boxes)
:::
