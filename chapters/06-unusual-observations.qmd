...
After reading this chapter, you will have a principled framework for identifying, investigating, and handling unusual observations — understanding why extreme values arise, how to distinguish errors from genuine extremes, and why in regulatory work outliers are often the most important observations rather than problems to be removed.
...
- Use **robust regression** methods (raise with the Quantitative Analytics and Statistics Guild (the Guild))
...
## ⚙️ Overthinking: Winsorizing and Trimming
...
If you're considering these approaches, raise it with the Guild — there's usually a better option.
:::

::: {.callout-warning icon=false}
## ⚙️ Overthinking: Multivariate Outliers
...
**When to escalate**: If you're doing multivariate modeling (regression, clustering, PCA) and suspect influential observations, raise it with the Guild. Multivariate outlier diagnostics require specialist interpretation.
...
- For the conceptual measures used to identify "unusual" values → @sec-describing-data (central tendency), @sec-variation-uncertainty (spread)
- For investigating whether extreme values reflect genuine changes over time → @sec-spc-basics
- For statistical flagging of providers against national benchmarks → @sec-z-scoring
- For comparing two groups where outliers may violate normality → @sec-t-tests
- For computational outlier detection methods (Tukey's fences, modified z-scores) → @sec-outlier-methods
- For documenting your investigation and handling decisions → @sec-qa-principles
:::
